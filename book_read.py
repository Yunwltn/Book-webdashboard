...
def run_book_read_app() :
    ...
    st.write('2020년도 국립중앙도서관의 연간 독서량 파일을 활용하여')
    ...
    if st.checkbox('월별 연령대 평균량 알아보기') :
        ...
        if my_choice != 0 :
            ...
            fig1 = px.bar(choice_Month, y='독서량', height=600, title= str(my_choice)+ '월 연령대별 평균 독서량')
    ...
    fig2 = px.bar(AREA_AGEdf, x= '연령', y=['회원수','대출건수'], barmode='group', height=600)
    ...
    if status == '지역별 평균 독서량' :
        ...
        fig3 = px.bar(AREA_READQY, y='독서량', height=600, title= '지역별 평균 독서량')
    ...
    if st.checkbox('각 지역별 세부 데이터 보기') :
        ...
        if my_choice != 0 :
            ...
            st.write(my_choice + '의 총 회원 수는' + str(df2['회원수'].sum()) + '명 입니다')
